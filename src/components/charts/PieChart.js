...
function PieChart({ chartData }) {
  ...
  const legend = colors.map((color, index) => (
    <g key={index} transform={`translate(${centerX + radius + 10},${centerY - radius + 20 + index * 20})`}>
  ...
  return (
    <svg width={400} height={400} viewBox="0 0 400 400">
      {pieSegments}
      <text x={centerX} y={centerY + radius + 30} textAnchor="middle" dominantBaseline="middle" fontSize="16">
